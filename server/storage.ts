...
export class MemStorage implements IStorage {
  ...
  async createOTP(identifier: string, type: "phone" | "email", otp: string, purpose: string, expiresAt: Date): Promise<OTPRecord> {
    const id = randomUUID();
    // Normalize identifier: lowercase for email, trim for both
    ...
    const record: OTPRecord = {
      id,
      phone: type === "phone" ? normalizedIdentifier : null,
      email: type === "email" ? normalizedIdentifier : null,
      otp: String(otp).trim(), // Ensure OTP is stored as string
      purpose,
      expiresAt,
      verified: false,
      createdAt: new Date(),
  ...
  async getOTP(identifier: string, type: "phone" | "email", purpose: string): Promise<OTPRecord | undefined> {
    // Normalize identifier for comparison (lowercase for email, trim for both)
    const normalizedIdentifier = type === "email" 
      ? identifier.trim().toLowerCase() 
      : identifier.trim();
    ...
    return Array.from(this.otpRecords.values())
      .filter(r => {
        const recordIdentifier = type === "phone" 
          ? (r.phone || "").trim() 
          : (r.email || "").trim().toLowerCase();
        return recordIdentifier === normalizedIdentifier && r.purpose === purpose && !r.verified;
      })
    ...
    return Array.from(this.otpRecords.values())
      .filter(r => {
        const recordIdentifier = type === "phone" 
          ? (r.phone || "").trim() 
          : (r.email || "").trim().toLowerCase();
        return recordIdentifier === normalizedIdentifier && r.purpose === purpose;
      })
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
  }
