...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/verify-otp", async (req: Request, res: Response) => {
    try {
      ...
      if (!record) {
        console.log(`[Verify OTP] No record found for ${identifier} (${type}) purpose=${data.purpose || "login"}`);
        // Debug: log all OTP records to help diagnose
        const allRecords = Array.from((storage as any).otpRecords?.values() || []);
        console.log(`[Verify OTP] Total OTP records: ${allRecords.length}`);
        const matchingRecords = allRecords.filter((r: any) => {
          if (type === "email") {
            return (r.email || "").toLowerCase() === identifier.toLowerCase();
          } else {
            return (r.phone || "").trim() === identifier.trim();
          }
        });
        console.log(`[Verify OTP] Records matching identifier: ${matchingRecords.length}`);
        if (matchingRecords.length > 0) {
          console.log(`[Verify OTP] Matching records:`, matchingRecords.map((r: any) => ({
            purpose: r.purpose,
            verified: r.verified,
      ...
      // Compare OTP as strings, trimming whitespace
      const enteredOTP = data.otp.trim();
      const storedOTP = String(record.otp).trim();
      
      if (storedOTP !== enteredOTP) {
        console.log(`[Verify OTP] Mismatch. Expected: "${storedOTP}", Got: "${enteredOTP}"`);
