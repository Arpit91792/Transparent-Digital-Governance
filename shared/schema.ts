...
export const insertUserSchema = baseInsertUserSchema
  ...
  .refine(
    (data) => {
      // If phone is provided, it should be 10 digits
      if (data.phone && data.phone.trim().length > 0) {
        return /^[0-9]{10}$/.test(data.phone.trim());
      }
      return true;
    },
    {
  ...
  .transform((data) => {
    ...
    return {
      ...data,
      email: data.email && data.email.trim().length > 0 ? data.email.trim().toLowerCase() : null,
